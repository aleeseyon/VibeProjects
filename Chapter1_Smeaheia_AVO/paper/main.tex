...
This paper asks a narrow, quantitative question: how large must relative errors in the caprock and reservoir P-velocities be before the AVO class of the Smeaheia top reservoir changes, and how much of the answer depends on the reflectivity approximation used? We make five contributions:
\begin{enumerate}
...
\item a symbolic second-order expansion of the exact gradient that identifies the origin of the Shuey bias and places the quadratic approximation of \citet{wang1999} within the complete second-order result.
\end{enumerate}
...
\label{eq:BAR}
\end{equation}
of which Shuey's expression is a re-parameterisation that agrees to first order in the contrasts.

\subsection*{Second-order expansion of the gradient}

To explain the differences between the gradient estimators, we expand them in the mean-referenced relative contrasts $a=\Delta\VP/\bar V_\mathrm{P}$, $b=\Delta\VS/\bar V_\mathrm{S}$, and $r=\Delta\rho/\bar\rho$, at fixed $k=\bar V_\mathrm{S}/\bar V_\mathrm{P}$. The layer values are $X_{1,2}=\bar X(1\mp x/2)$, with $x$ the relative contrast of $X$. A symbolic series expansion of the exact tangent gradient (Appendix~A) gives
\begin{equation}
\Btan=\BAR+\underbrace{a\,\BAR}_{\text{reference angle}}+\underbrace{k^3(r+2b)^2}_{\text{shear}}\;\underbrace{-\;\tfrac{k}{4}\,r^2}_{\text{density}}+O(\chi^3),
\label{eq:B2}
\end{equation}
where $\chi$ denotes the magnitude of the contrasts. The same expansion of the Shuey gradient (equation~\ref{eq:B}) gives
\begin{equation}
B_\mathrm{Shuey}=\BAR+O(\chi^3),
\label{eq:Bsh}
\end{equation}
that is, the Shuey gradient has no second-order term at all.

\citet{wang1999} derived a quadratic approximation of $\RPP$, his equation 10. In terms of the mean of the incidence and transmission angles, $\bar\theta$ (his equation 6), its only second-order term is $(\bar\beta/\bar\alpha)^3\cos\bar\theta\sin^2\bar\theta\,(\Delta\rho/\bar\rho+2\Delta\beta/\bar\beta)^2$. The gradient of this term is exactly the shear term of equation~\ref{eq:B2}. Wang obtains his approximation after assuming $(\Delta\rho/\bar\rho)^2\approx0$ (his equation A-10), which removes the density term. The reference-angle term arises because the linearised formulas are written in $\bar\theta$, whereas the AVO gradient is measured against the incidence angle; since $\sin^2\bar\theta\approx(\bar V_\mathrm{P}/V_{\mathrm{P}1})^2\sin^2\theta$ at small angles, converting between the two multiplies the gradient by a positive factor. Equations~\ref{eq:B2} and \ref{eq:Bsh} were derived with SymPy from the explicit solution of Appendix~A and from equation~\ref{eq:B}. They were verified numerically by scaling the Smeaheia contrasts by $s=1$ to $1/16$: the residuals decrease as $s^3$ and converge to the symbolic third-order coefficients \nb{\S11.1--11.2}.
...
\subsection*{Origin of the Shuey bias}

Table~\ref{tab:dec} decomposes the two baseline gradients by order in the contrasts \nb{\S11.3}. The exact tangent gradient is almost entirely first plus second order. Aki--Richards ($-0.04450$) plus the complete second-order term ($+0.01455$) gives $-0.02996$, against the exact $-0.02994$. The largest second-order contribution is the reference-angle term ($+0.01272$), followed by the shear term of \citet{wang1999} ($+0.00462$) and the density term ($-0.00279$). The Shuey gradient has no second-order contribution. Its third-order term, however, is large ($+0.02163$) and overshoots the missing second-order correction. The net result is a gradient $0.00882$ closer to $B=0$ than the exact value. The bias of the Shuey gradient toward the class boundary is therefore a third-order artifact of its Poisson's-ratio parameterisation, not an incomplete second-order correction.

\begin{table}[htbp]
\centering
\caption{Decomposition of the baseline gradient by order in the relative contrasts ($a=-0.2857$, $b=-0.0168$, $r=-0.1461$, $k=0.5231$) \nb{\S11.3}.}
\label{tab:dec}
\begin{tabular}{lrr}
\toprule
Contribution & Exact tangent $\Btan$ & Two-term Shuey \\
\midrule
First order ($\BAR$) & $-0.04450$ & $-0.04450$ \\
Second order: reference angle, $a\BAR$ & $+0.01272$ & \multirow{3}{*}{$0$} \\
Second order: shear, $k^3(r+2b)^2$ & $+0.00462$ & \\
Second order: density, $-kr^2/4$ & $-0.00279$ & \\
Third order & $+0.00037$ & $+0.02163$ \\
Fourth and higher orders & $-0.00035$ & $+0.00176$ \\
\midrule
Total & $-0.02994$ & $-0.02111$ \\
\bottomrule
\end{tabular}
\end{table}

The second-order gradients also reproduce the class boundary (Table~\ref{tab:thr2}) \nb{\S11.4}. The complete second-order gradient places the $B=0$ threshold within 0.12 percentage points of the exact tangent value. Wang's equation 10 places it 0.19--0.20 points lower. Because converting between $\bar\theta$ and the incidence angle multiplies Wang's entire gradient by a positive factor, the choice of angle changes its baseline value ($-0.03053$ when converted to the incidence angle, $-0.03988$ when $\bar\theta$ is read as the incidence angle) but not its threshold. The remaining shortfall is due to the omitted density term.

\begin{table}[htbp]
\centering
\caption{Baseline gradient and $B=0$ threshold $|r|$ (\%) for the second-order gradients, with the exact tangent and two-term Shuey values for reference \nb{\S11.4}.}
\label{tab:thr2}
\begin{tabular}{lrrr}
\toprule
Estimator & $B_0$ & $\phi=135^\circ$ & Min.\ $|r|$ ($\phi$) \\
\midrule
Exact, to second order (equation~\ref{eq:B2}) & $-0.02996$ & 5.876 & 5.697 (149.2\textdegree) \\
\citet{wang1999}, equation 10, incidence angle & $-0.03053$ & 5.554 & 5.390 (149.0\textdegree) \\
Zoeppritz tangent (exact) & $-0.02994$ & 5.755 & 5.580 (149.2\textdegree) \\
Two-term Shuey & $-0.02111$ & 4.752 & 4.444 (156.6\textdegree) \\
\bottomrule
\end{tabular}
\end{table}
...
The conclusion drawn from two-term Shuey reflectivity survives exact reflectivity: differential $\VP$ errors of a few percent can change the apparent AVO class at the Smeaheia top reservoir. Its magnitude does not survive unchanged. At the contrasts of this interface ($\Delta\VP/\bar V_\mathrm{P}\approx-0.29$), the second-order correction to the gradient ($+0.0146$) is a third of the first-order value. Aki--Richards omits it, and Shuey replaces it with a larger third-order term, so the two bracket the exact tangent gradient from opposite sides (Tables~\ref{tab:thr} and \ref{tab:dec}). Two-term Shuey therefore overstates the vulnerability of the classification by 17\%--20\% in required error magnitude, and it produces spurious crossings for reservoir-only and common-mode errors. Where a closed-form gradient is preferred to the exact solution, equation~\ref{eq:B2} is the appropriate choice at contrasts of this size: it reproduces the exact gradient to $2\times10^{-5}$ and the class threshold to about 0.1 percentage points. Wang's (1999) equation 10 is nearly as accurate, provided its mean angle is converted to the incidence angle when a gradient value is reported.
...
The origin of this bias is identified by a symbolic second-order expansion. The exact gradient equals the Aki--Richards gradient plus a reference-angle, a shear, and a density term; to second order this reproduces the exact gradient at Smeaheia to $2\times10^{-5}$. The Shuey gradient has no second-order term, and its proximity to the exact value comes from a third-order term that overshoots.
